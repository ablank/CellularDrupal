  ...
  Drupal.behaviors.cellular = {
   ...
   win.type =  content.match(/\w*[^\"\'](?=-)/g).join("");
      ...
      cellular.windowstate = cellular.debounce(function () {
        const ob = cellular.state.breakpoint;
       ...
       */
      cellular.scrollstate = cellular.debounce(
        function (e, y) {
          const el = jQuery("body");
          const cclass = "scrolling";
          const uc = `${cclass}-up`;
          const dc = `${cclass}-down`;
          const dst = jQuery(document).scrollTop();
          const region = el.height() / 3;
          ...
          // % of doc scrolled
          cellular.state.scrolled =
            (dst / (el.height() - jQuery(window).height())) * 100;
        },
        0,
        true
      );
      ...
      cellular.jFormal = function (opts) {
        ...
        return this.each(function () {
          const inputs = o.inputs.join(",");
          // get/set value of inputs
          jQuery(inputs).each(function () {
            const $t = jQuery(this);
            const hold = (holder = $t.attr("placeholder"));
            $t.on("focus", function () {
              holder = "";
      ...
      cellular.jMmenu = function (opts) {
        ...
        fn.mediaQuery = cellular.debounce(function ($obj, state) {
          ...
          if (o.breakpoint === cellular.state.breakpoint) {
            ...
            o.parent.removeClass(
              `${o.cclass}-active ${o.cclass}-inactive ${o.animateclass}`
            );
            $obj.attr("aria-label", "Menu").removeClass(`${o.cclass}-trigger`);
            jQuery(`.${o.cclass}-menu`)
              .removeClass(`${o.cclass}-menu`)
              .prependTo($obj);
        ...
        fn.menutrigger = function ($obj, state) {
          const classes = [`${o.cclass}-active`, `${o.cclass}-inactive`];
        ...
        fn.style = function ($obj) {
          const menu = $obj.find(">ul");
          const nested = menu.find("ul");

          // Add classes for parent/child.
          if (nested.length > 0) {
            const child = menu.find("li ul");
        ...
        fn.listen = function ($obj, state) {
          ...
          jQuery(".parent > a").click(function (e) {
            if (state.mmenu) {
              ...
              const child = parent.children(":gt(0)");
      ...
      cellular.jScrolli = function (opts) {
        ...
        fn.button = function ($text) {
          return `<a class="${
            o.cclass
          }-control ${$text.toLowerCase()}">${$text}</a>`;
        };
        ...
        fn.go = function (index, $obj, state) {
          if (!state.paused) {
            const tclass = "transition";
            const li = $obj.find(`.${o.cclass}-slide`);
            // Unset tabindexes
            ...
            jQuery(li[index]).activate().find("a").prop("tabindex", "-1");
        ...
        fn.caption = function ($obj, state) {
          const wrap = $obj.parent().parent();
          const cap = wrap.find("> .caption p");
          // Get current slide's caption
          state.caption = wrap
            .find(o.caption.selector)
            .eq(state.current)
            .text();
        ...
        fn.events = function ($obj, state) {
          const controls = $obj.siblings(".controls");
     ...
     */
          ...
          jQuery(document).ready(function () {
            if (o.height === "auto") {
              $obj.find("> li").each(function () {
                const tHeight = this.clientHeight;
        ...
        fn.style = function ($obj, state) {
          ...
          li.prop("tabindex", "-1")
            ...
            .each(function () {
              const $t = jQuery(this);
              $t.children().wrapAll(
                `<div class="${o.cclass}-slide-content cell" />`
              );
              ...
              if (o.background) {
                const background = $t.find(o.background);
          ...
          if (o.controls.showmarkers) {
            const markers = jQuery(`<ul class="${o.cclass}-markers"/>`);
            for (let i = 0; i < li.length; i += 1) {
              markers.append(
                `<li class="${o.cclass}-marker" data-href="${i}">${i + 1}</li>`
              );
            }
            $obj.after(markers);
            fn.mark($obj, state);
          ...
          if (o.caption.enable) {
            const cap = $obj.find(o.caption.selector);
       ...
       */
        ...
        fn.style = function ($obj) {
          $obj.once("jSocial", function () {
            if (o.share) {
              const sWrap = $('<div class="jSocial-share" />');
              let sharetitle;
              const sharelinks = {
                facebook: {
                  title: "Facebook",
              ...
              o.share.map(function (i) {
                sWrap.buttonize(
                  sharelinks[i].url,
                  sharetitle + sharelinks[i].title,
                  [sharelinks[i].title.toLowerCase(), o.buttonclass, "icon"]
                );
              });
            ...
            if (Object.keys(o.follow) !== "undefined") {
              const fWrap = $('<div class="jSocial-follow" />');
              let followtitle = "";
              /*
                 facebook: {
                 title: "Facebook",
      ...
      cellular.jTabs = function (opts) {
        ...
        fn.init = function () {
          ...
          const wrap = jQuery("<div/>").classify([
            o.orient,
            `${o.cclass}-wrap`,
          ]);
          const panel =
