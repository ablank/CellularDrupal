cellular.jAV = function (opts) {
  ...
  fn.getControls = function ($obj) {
    const block = $obj.find(`.${o.cclass}-controls`);
    const controls = {
  ...
  fn.youtube = function ($obj, link) {
    let youtube;
    const controls = fn.getControls($obj);
    let vid;
    // match(\/youtu.be\/(.*))
    ...
    function onYouTubeIframeAPIReady() {
      const vid = link.match(/\/watch\?v=(.*)/);
      const uPlaylist = o.playlist;
      const youtube = new YT.Player($obj.find(o.type).attr("id"), {
  ...
  fn.style = function ($obj) {
    let media = jQuery(`<${o.type}/>`, {
      id: `${o.cclass}-${uuid++}`,
    });
    const overlay = jQuery(`<div class="${o.cclass}-overlay" />`);
    let controls = "";
    let source = $obj.attr("data-src") || o.source;
    ...
    if (typeof (source !== "undefined")) {
      ...
      source.map(function (el) {
        if (
          el.indexOf("https://www.youtube.com") !== -1 ||
          el.indexOf("https://youtu.be") !== -1
        ) {
          media = $obj;
          fn.youtube($obj, el);
    ...
    if (o.opt.showControls === true) {
      ...
      for (const control in o.controls) {
        if (o.controls.hasOwnProperty(control)) {
          const btn = jQuery(`<a>${o.controls[control].text}</a>`)
            .classify([control])
            .attr({
      ...
      if (typeof o.controls.volume !== "undefined") {
        $obj.find(".volume").append('<div class="volume-handle" />');
      }
    }

    $obj.append(overlay, media, controls);
    // console.log($obj.attr('id'));
    for (const attribute in o.attributes) {
  ...
  fn.events = function ($obj, state) {
    const media = document.getElementById($obj.find(o.type).attr("id"));
    const controls = fn.getControls($obj);
    let $t;
  ...
  fn.init = function () {
    ...
    const state = {
      ...
      fullscreen: false,
      buffering: false,
      playing: (o.attributes.autoplay = !!true),
      paused: (o.attributes.autoplay = !true),
      muted: false,
