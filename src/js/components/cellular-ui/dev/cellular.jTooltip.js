cellular.jTooltip = function (opts) {
  ...
  fn.style = function ($obj) {
    const tooltip = jQuery(`<div>${$obj.attr(o.dataattr)}</div>`);
    const tclass = `${o.cclass}-trigger`;
    ...
    if (o.triggerbtn === true) {
      const btn = jQuery(`<span aria-label="${o.triggerbtntext}">?</span>`);
      const tindex = $obj.prop("tabindex") ? $obj.prop("tabindex") : "0";

      btn.classify([tclass, `${o.cclass}-btn`]).prop("tabindex", tindex);
      $obj.wrap(`<div class="${o.cclass}-wrap" />`).parent().prepend(btn); // .before(btn)
    } else {
      $obj.addClass(tclass);
    }
    // Add the tooltip.
  ...
  fn.listen = function ($obj) {
    jQuery(`.${o.cclass}-trigger`)
      .on("mouseenter focus", function (e) {
        const $t = jQuery(this);
        const tooltip = $t.nextAll(`.${o.cclass}:first`);
        let btn = {};
        let position = {};
