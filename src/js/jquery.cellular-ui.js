...
cellular.scrollstate = cellular.debounce(
  function (e, y) {
    var el = jQuery("body");
    var cclass = "scrolling";
    var uc = `${cclass}-up`;
    var dc = `${cclass}-down`;
    var dst = jQuery(document).scrollTop();
    var region = el.height() / 3;
...
cellular.jMmenu = function (opts) {
  ...
  fn.init = function () {
    ...
    $obj.addClass(o.cclass).one(o.cclass, fn.style($obj));
...
cellular.jScrolli = function (opts) {
  ...
  fn.go = function (index, $obj, state) {
    if (!state.paused) {
      ...
      var li = $obj.find(`.${o.cclass}-slide`);
      // Unset tabindexes
